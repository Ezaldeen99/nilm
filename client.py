...
import metrics
from gen import get_test_data, generate_batch, generate_dataset
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python3 client.py device_name")
        exit()
    key_name = sys.argv[1]

    # if user didn't generate the dataset, let's try to generate the dataset
    if not os.path.exists("dataset/ground_truth_and_mains") or not os.path.exists("dataset/trainsets"):
        generate_dataset("data/ukdale.h5")
